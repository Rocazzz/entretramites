...
const FAQS = [
  { q: "¿Cuánto tarda el proceso de aprobación?", a: "El visado de emprendedor suele resolverse entre 10 y 20 días hábiles desde la presentación del expediente completo. La autorización de residencia (si ya estás en España) puede tardar hasta 20 días hábiles adicionales." },
          ...
          {FAQS.map(({ q, a }, i) => (
            <div key={i} className={`faq__item ${open === i ? "faq__item--open" : ""}`}>
              <button className="faq__question" onClick={() => setOpen(open === i ? null : i)} aria-expanded={open === i}>
                ...
                <span className="faq__icon" style={{ transform: open === i ? "rotate(45deg)" : "rotate(0)" }}>+</span>
              </button>
